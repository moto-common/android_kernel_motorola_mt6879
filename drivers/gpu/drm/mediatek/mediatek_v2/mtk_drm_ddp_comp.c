...
#define MT6739_HRT_URGENT_CTL_SEL_ALL             REG_FLD_MSB_LSB(7, 0)
	#define MT6739_HRT_URGENT_CTL_SEL_RDMA0       REG_FLD_MSB_LSB(0, 0)
	#define MT6739_HRT_URGENT_CTL_SEL_WDMA0       REG_FLD_MSB_LSB(2, 2)
	#define MT6739_HRT_URGENT_CTL_SEL_DSI0        REG_FLD_MSB_LSB(5, 5)

#define MT6739_HRT_URGENT_CTL_VAL_ALL             REG_FLD_MSB_LSB(15, 8)
	#define MT6739_HRT_URGENT_CTL_VAL_RDMA0       REG_FLD_MSB_LSB(8, 8)
	#define MT6739_HRT_URGENT_CTL_VAL_WDMA0       REG_FLD_MSB_LSB(10, 10)
	#define MT6739_HRT_URGENT_CTL_VAL_DSI0        REG_FLD_MSB_LSB(13, 13)

#define MT6765_HRT_URGENT_CTL_SEL_ALL             REG_FLD_MSB_LSB(7, 0)
	#define MT6765_HRT_URGENT_CTL_SEL_RDMA0       REG_FLD_MSB_LSB(0, 0)
	#define MT6765_HRT_URGENT_CTL_SEL_WDMA0       REG_FLD_MSB_LSB(1, 1)
			    ...
			    struct cmdq_pkt *handle, void *data)
{
	...
	if (id == DDP_COMPONENT_ID_MAX) { /* config when top clk on */
		...
		SET_VAL_MASK(sodi_req_val, sodi_req_mask,
					0, MT6739_SODI_REQ_MASK_ALL);

		/* apply sodi hrt with rdma fifo*/
		SET_VAL_MASK(sodi_req_val, sodi_req_mask,
					1, MT6739_SODI_REQ_MASK_RDMA0);
		SET_VAL_MASK(sodi_req_val, sodi_req_mask,
					0, MT6739_DVFS_HALT_MASK_SEL_ALL);
	} else if (id == DDP_COMPONENT_RDMA0) {
		SET_VAL_MASK(sodi_req_val, sodi_req_mask, (!en),
					MT6739_SODI_REQ_MASK_RDMA0);
		SET_VAL_MASK(sodi_req_val, sodi_req_mask, en,
					MT6739_DVFS_HALT_MASK_SEL_RDMA0);
	} else if (id == DDP_COMPONENT_WDMA0) {
		SET_VAL_MASK(sodi_req_val, sodi_req_mask, en,
					MT6739_DVFS_HALT_MASK_SEL_WDMA0);
	} else
		return;
	...
	if (handle == NULL) {
		...
		v = (readl(priv->config_regs + MT6739_MMSYS_SODI_REQ_MASK)
			& (~sodi_req_mask));
		v += (sodi_req_val & sodi_req_mask);
		writel_relaxed(v, priv->config_regs + MT6739_MMSYS_SODI_REQ_MASK);
	} else {
		cmdq_pkt_write(handle, NULL, priv->config_regs_pa +
			MT6739_MMSYS_SODI_REQ_MASK, sodi_req_val, sodi_req_mask);
	}
}
